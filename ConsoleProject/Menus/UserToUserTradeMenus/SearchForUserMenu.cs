...
namespace ConsoleProject.Menus.UserToUserTradeMenus
{
    public class SearchForUserMenu
    {
        public static void SearchForOtherUser(User user)
        {
            ...
            if(DBContext.Users.Any(x => x.Email == searchedEmail) && DBContext.Users.Single(x => x.Email == searchedEmail).PrivateProfile == false)
            {
                var searchedUser = DBContext.Users.Single(x => x.Email == searchedEmail);
